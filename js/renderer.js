// ════════════════════════════════════════
//  renderer.js — 캔버스 렌더링
//  미러링 없이 HOST/JOIN 동일하게 렌더링
//  JOIN HUD만 "나=왼쪽" 기준으로 표시
// ════════════════════════════════════════
...
function drawArena(a, players){
  ...
  for(let x=a.x;x<=a.x+a.w;x+=60){ctx.beginPath();ctx.moveTo(x,a.y);ctx.lineTo(x,a.y+a.h);ctx.stroke();}
  for(let y=a.y;y<=a.y+a.h;y+=60){ctx.beginPath();ctx.moveTo(a.x,y);ctx.lineTo(a.x+a.w,y);ctx.stroke();}
  ctx.restore();
  ...
  // 영역 라벨 — JOIN이면 내 진영(오른쪽=P2)에 "MY TERRITORY"
  ctx.font="bold 11px 'Cinzel',serif"; ctx.textAlign='center'; ctx.textBaseline='top';
  if(netRole==='join'){
    ctx.fillStyle='rgba(255,107,53,.2)';  ctx.fillText('MY TERRITORY',   a.x+a.w*3/4, a.y+8);
    ctx.fillStyle='rgba(74,240,255,.15)'; ctx.fillText('ENEMY TERRITORY',a.x+a.w/4,   a.y+8);
  } else {
